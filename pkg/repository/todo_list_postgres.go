...
func (r *TodoListPostgres) Create(userId int, list todo.TodoList) (int, error) {
	...
	row := tx.QueryRow(createListQuery, list.Title, list.Describtion)
...
func (r *TodoListPostgres) GetById(userId int, listId int) (todo.TodoList, error) {
	...
	query := fmt.Sprintf("SELECT tl.id, tl.title, tl.descriptions FROM %s tl INNER JOIN %s ul ON tl.id = ul.list_id WHERE ul.user_id = $1 AND tl.id=$2",
		todoListsTable, usersListsTable)
